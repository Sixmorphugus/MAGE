...
namespace mage {
	...
	template<typename T>
	inline bool line<T>::intersects(point<T>& point) const
	{
		auto end = getEndPoint();
		...
		if (end.x <= max(start.x, point.x) && end.x >= min(start.x, point.x) &&
			end.y <= max(end.y, point.y) && end.y >= min(start.y, point.y))
	...
	template<typename T>
	inline bool line<T>::intersects(line<T>& point) const
	{
		// from geeksforgeeks.com
		// http://www.geeksforgeeks.org/check-if-two-given-line-segments-intersect/
		...
		// special cases
		int o1 = orientation(start, getEndPoint(), point.start);
		int o2 = orientation(start, getEndPoint(), point.getEndPoint());
		int o3 = orientation(point.start, point.getEndPoint(), start);
		int o4 = orientation(point.start, point.getEndPoint(), getEndPoint());
	...
	template<typename T>
	inline point<T> line<T>::getEndPoint()
	{
		return start + end.to3();
	}
}